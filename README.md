...
zig create [name]               Create a new workflow interactively with an AI agent
...
zig create --output workflows/ci.zug
```
...
A `.zug` file is a TOML workflow definition that describes a DAG of AI agent steps with shared variables, conditional routing, and data flow. It is generated by `zig create` and executed by `zig run`. The format captures:
