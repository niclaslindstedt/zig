...
| File | Covers |
|------|--------|
| `zug-vs-zag-gap-analysis.md` | Gap analysis between .zug format capabilities and zag features |
...
|------------------------------|-----------------|
| `zig-core/src/workflow/model.rs` (new fields, types) | `zug-vs-zag-gap-analysis.md` (supported features table) |
| `zig-core/src/workflow/parser.rs` (format changes) | `zug-vs-zag-gap-analysis.md` |
| `zig-core/src/run.rs` (execution changes) | `zug-vs-zag-gap-analysis.md` (zag mapping column) |
| `zig-cli/src/cli.rs` (Pattern enum) | `zug-vs-zag-gap-analysis.md` (pattern support) |
| New zag features adopted | `zug-vs-zag-gap-analysis.md` (close gaps) |
| New .zug fields | `zug-vs-zag-gap-analysis.md` (add to supported table) |
...
When a .zug feature is implemented that was previously listed as a gap:
...
When a new zag feature is discovered that .zug doesn't support:

1. Add to the appropriate gap category
2. Document what the zag feature does and why it might be useful
3. Suggest a potential .zug syntax if applicable
...
- [ ] Update `zug-vs-zag-gap-analysis.md` if workflow model, parser, or execution changed
