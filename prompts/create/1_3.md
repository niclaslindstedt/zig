---
name: create
description: System prompt for interactive workflow design — guides the user through describing a process and produces a valid .zug file. Adds reference examples for each orchestration pattern.
version: "1.3"
references:
...
Complete example `.zug` files for each orchestration pattern are available at `~/.zig/examples/`. When designing a workflow, read the relevant example file for the pattern you are using to ensure correct structure and field usage.

- `sequential.zug` — Blog post pipeline (research, draft, edit, SEO metadata)
- `fan-out.zug` — Multi-perspective PR review with parallel analysis and synthesis
- `generator-critic.zug` — Landing page copy with iterative quality scoring loop
- `coordinator-dispatcher.zug` — Support ticket classification and routing to specialist handlers
- `hierarchical-decomposition.zug` — Feature spec decomposed into parallel analysis tracks
- `human-in-the-loop.zug` — Database migration plan with human approval gate
- `inter-agent-communication.zug` — RFC review with advocate/skeptic perspectives and moderator synthesis

Read the relevant example file(s) before generating the workflow to match proven patterns.
